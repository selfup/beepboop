export class Note {
    ...
    static all() {
        return [
            new Note('C'), new Note('C#'), new Note('D'), new Note('D#'), new Note('E'),
            new Note('F'), new Note('F#'), new Note('G'), new Note('G#'),
            new Note('A'), new Note('A#'), new Note('B')
        ];
    }
    ...
    transpose(semitones) {
        const allNotes = Note.all();
        const currentIndex = allNotes.findIndex(note => note.name === this.name);
        const newIndex = (currentIndex + semitones + allNotes.length) % allNotes.length;
